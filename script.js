...
// If running locally (VS Code Live Server, file protocol, etc.), point to backend port 8001
if (window.location.protocol === 'file:' ||
    window.location.hostname === 'localhost' ||
    window.location.hostname === '127.0.0.1') {

    // Only override if we are NOT already on the backend port
...
function displayRecipe(recipe) {
    ...
    // Set recipe image - clear old image first to prevent showing wrong dish
    const recipeImageNew = document.querySelector('#recipeDisplay #recipeImage');

    // Clear the old image immediately and show a loading placeholder
    recipeImageNew.style.opacity = '0.3';
    recipeImageNew.src = 'https://via.placeholder.com/900x400/6366f1/ffffff?text=Loading+Image...';
    recipeImageNew.alt = 'Loading...';

    // Clear any previous error handlers
    delete recipeImageNew.dataset.triedUnsplash;
    delete recipeImageNew.dataset.triedLorem;

    // Preload the new image
    const newImageUrl = recipe.image_url || `https://source.unsplash.com/1600x900/?${encodeURIComponent(recipe.name)},food`;
    const tempImage = new Image();

    tempImage.onload = function () {
        // Image loaded successfully, display it
        recipeImageNew.src = newImageUrl;
        recipeImageNew.alt = recipe.name || 'Recipe';
        recipeImageNew.style.opacity = '1';
    };

    tempImage.onerror = function () {
        // Try fallback sources
        const fallbackUrl = `https://source.unsplash.com/1600x900/?${encodeURIComponent(recipe.name)},food`;
        const fallbackImage = new Image();

        fallbackImage.onload = function () {
            recipeImageNew.src = fallbackUrl;
            recipeImageNew.alt = recipe.name || 'Recipe';
            recipeImageNew.style.opacity = '1';
        };
        ...
        fallbackImage.onerror = function () {
            ...
            recipeImageNew.src = 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=900&q=80';
            recipeImageNew.alt = recipe.name || 'Recipe';
            recipeImageNew.style.opacity = '1';
        };

...
async def get_food_image(dish_name: str, meal_thumb: Optional[str] = None) -> str:
    """Fetch image - prioritize TheMealDB real photos, then AI-generated images for everything else."""
    
    # If we have TheMealDB image, use it (fastest, most reliable, REAL photo)
    if meal_thumb and meal_thumb.strip():
        return meal_thumb
    
    # Check cache (for AI-generated or Pexels images from previous searches)
    ...
    # Use Pexels API for REAL food photography
    PEXELS_API_KEY = os.getenv("PEXELS_API_KEY")
    
    # Determine cuisine type for better search
    dish_lower = dish_name.lower()
    
    if any(word in dish_lower for word in ["dosa", "idli", "biryani", "dal", "curry", "masala", "paneer", "chapati", "naan", "tandoori", "samosa"]):
        cuisine_type = "indian"
    elif any(word in dish_lower for word in ["pizza", "pasta", "gnocchi", "lasagna", "risotto", "spaghetti"]):
        cuisine_type = "italian"
    elif any(word in dish_lower for word in ["sushi", "ramen", "tempura", "teriyaki", "miso"]):
        cuisine_type = "japanese"
    elif any(word in dish_lower for word in ["taco", "burrito", "enchilada", "quesadilla"]):
        cuisine_type = "mexican"
    elif any(word in dish_lower for word in ["burger", "fries", "sandwich", "steak"]):
        cuisine_type = "american"
    else:
        cuisine_type = ""
    
    # Search queries: ONLY exact dish searches, no generic fallbacks
    search_queries = [
        dish_name,  # Try exact dish name first
        f"{cuisine_type} {dish_name}" if cuisine_type else dish_name,  # Cuisine + dish
        f"{dish_name} food"  # Dish + food
    ]
            ...
            async with httpx.AsyncClient() as client:
                for query in search_queries:
                    response = await client.get(
                        ...
                        params={
                            "query": query,
                            "per_page": 10,  # Get more results to ensure quality
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        # STRICT: Only accept if we have at least 3 good results
                        # This ensures the query is specific enough
                        if data.get("photos") and len(data["photos"]) >= 3:
                            # Use first photo
                            image_url = data["photos"][0]["src"]["large"]
                            API_CACHE[cache_key] = (image_url, time.time())
                            print(f"📸 Pexels: '{dish_name}' using query '{query}' ({len(data['photos'])} results)")
                            return image_url
        except Exception as e:
            print(f"Pexels API Error: {e}")
    
    # Final fallback: AI-generated food image using Pollinations.AI (free, no API key needed)
    # This generates a realistic food image based on the dish name
    try:
        print(f"🎨 Generating AI image for '{dish_name}'...")
        # Pollinations.AI free image generation - creates realistic food photos
        ai_prompt = f"professional food photography of {dish_name}, appetizing, high quality, restaurant style"
        ai_image_url = f"https://image.pollinations.ai/prompt/{requests.utils.quote(ai_prompt)}?width=1200&height=800&nologo=true"
        API_CACHE[cache_key] = (ai_image_url, time.time())
        print(f"✨ Using AI-generated image for '{dish_name}'")
        return ai_image_url
    except Exception as e:
        print(f"AI Image Generation Error: {e}")
        # Ultimate fallback: beautiful generic food image
        fallback_url = "https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg?auto=compress&cs=tinysrgb&w=800"
        API_CACHE[cache_key] = (fallback_url, time.time())
...
async def generate_recipe(request: RecipeRequest):
    ...
    # Fallback to Groq AI if TheMealDB doesn't have it
    if groq_client:
        try:
            print(f"Attempting to generate recipe for '{dish_name}' using Groq AI...")
...
{{
    ...
    "prep_time": "15 mins",
    "cook_time": "30 mins",
    "dietary_type": "Veg or Non-Veg"
}}

Return ONLY valid JSON, no markdown formatting."""
            
            # Fetch Groq data and image in parallel
            async def call_groq():
            ...
            groq_task = asyncio.create_task(call_groq())
            image_task = asyncio.create_task(get_food_image(dish_name))
            
            text, image_url = await asyncio.gather(groq_task, image_task)
            ...
            print(f"Successfully generated recipe for '{dish_name}' using Groq")
            return Recipe(
                id=f"groq-{dish_name.lower().replace(' ', '-')}",
                name=data["name"],
            ...
            print(f"Groq AI Error for '{dish_name}': {e}")
            import traceback
            traceback.print_exc()

    print(f"Failed to find or generate recipe for '{dish_name}'")
...
Return ONLY the JSON, no markdown formatting, no explanations."""
        ...
        response_text = await groq_task
        text = clean_json_text(response_text)
        ...
        return Recipe(
            id=f"groq-ingredient-{dish_name.lower().replace(' ', '-')}",
            name=dish_name,
